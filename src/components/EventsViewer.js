...
import AvailableDatesCalendar from "./AvailableDateCalendar";
import {
  postEditEvent,
  postEditFoodForm,
  postEditMeetingForm,
...
export default function EventsViewer() {
  ...
  useEffect(() => {
    axios
      .get("https://3.27.163.46/api/available/dates")
      .then((response) => {
        const adjustedDates = response.data.availableDates.map((dateObj) => {
        ...
        setAvailableDates(adjustedDates);
      })
      .catch((error) => {
  ...
  const handleDateChange = (date) => {
    ...
    axios
      .get(`https://3.27.163.46/api/time/available?date=${date}`)
      .then((response) => {
